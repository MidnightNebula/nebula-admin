...
import { Field, FieldGroup } from '@/app/_shared/shadcn/components/field';

export function LoginForm({ className, onSubmit, ...props }: React.ComponentProps<'form'>) {
  return (
    <form className={cn('flex flex-col gap-6', className)} {...props} onSubmit={onSubmit}>
      <FieldGroup>
        ...
        <Field>
          <Button name="action" value="discord" className="cursor-pointer" variant="outline" type="submit">
